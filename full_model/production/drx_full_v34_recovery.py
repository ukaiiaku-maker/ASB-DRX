...
P = dict(
    ...
    # Directive-v5 entropy separation.  Zero values reproduce exact v34.
    # Each entry is mechanism-specific ΔS*/kB and enters its rate once.
    glide_activation_entropy_kB=0.0,
    recovery_activation_entropy_kB=0.0,
    climb_activation_entropy_kB=0.0,
    wall_activation_entropy_kB=0.0,
    gb_source_activation_entropy_kB=0.0,
    gb_transmission_activation_entropy_kB=0.0,
    embryo_activation_entropy_kB=0.0,
    boundary_activation_entropy_kB=0.0,
    activation_negative_barrier_mode='drag',
    activation_drag_exponent_cap=40.0,
    activation_entropy_abs_max_kB=20.0,
...
def _activation_exponential_from_free_barrier(barrier_j, temperature_k, lower=-700.0):
    """Boltzmann factor with a declared branch for ΔG* < 0."""
    G = np.asarray(barrier_j, dtype=float)
    TT = np.maximum(np.asarray(temperature_k, dtype=float), 1.0)
    negative = G < 0.0
    mode = str(P.get('activation_negative_barrier_mode', 'drag')).lower()
    if np.any(negative) and mode not in ('drag', 'barrierless_drag'):
        raise ValueError('negative free barrier outside declared activation validity envelope')
    arg = -G/(kB_J*TT)
    drag_cap = float(P.get('activation_drag_exponent_cap', 40.0))
    arg = np.where(negative, drag_cap, arg)
    return np.exp(np.clip(arg, float(lower), drag_cap))
...
def _gb_mobility_factor_from_T(Tfield):
    ...
    arg = arg + float(P.get('boundary_activation_entropy_kB', 0.0))
    fac = np.exp(arg)
...
def _km_k2_from_T(Tfield):
    ...
    return P['KM_k2_0'] * np.exp(
        float(P.get('recovery_activation_entropy_kB', 0.0))
        - P['KM_Q2_eV']/(8.617333262145e-5*TT))
...
def _lattice_diffusive_recovery_rate(rho_total, Tfield):
    ...
    D_L = alloy * D0 * np.exp(np.clip(
        float(P.get('climb_activation_entropy_kB', 0.0))
        - QeV/(8.617333262145e-5*T_arr), -700.0, 100.0))
...
class ATPotential:
    ...
    def barrier_H(s, sig, T):
        """EXP-floor activation enthalpy H*(sigma,T) [J].
    ...
    def barrier_G(s, sig, T, entropy_over_kB=None):
        """Free barrier ΔG*=ΔH*-TΔS* [J]; entropy enters exactly once."""
        if entropy_over_kB is None:
            entropy_over_kB = float(P.get('glide_activation_entropy_kB', 0.0))
        return (s.barrier_H(sig, T)
                - np.asarray(T, dtype=float)*ARRHENIUS_KB_J_K*float(entropy_over_kB))
    ...
    def S_over_kB(s, sig, T):
        ...
        if s.barrier_name() == 'kaa':
            ...
            Gp = s.barrier_H(sig, T + dT)
            Gm = s.barrier_H(sig, np.maximum(T - dT, 1.0))
    ...
    def _collective_fields(s, sig, rho, T):
        ...
        G = s.barrier_G(sig, T)
        h1 = s.eta0 * _activation_exponential_from_free_barrier(G, T)
    ...
    def gdot_independent(s, sig, rho, T):
        ...
        return pref * _activation_exponential_from_free_barrier(G, T)
...
def _restore_candidate_restart_state(path):
    """Restore the complete v34 candidate arrays or reject an inexact restart."""
    global nuc_cand_active, nuc_cand_age, nuc_cand_best_barrier, nuc_cand_birth_step
    if path is None or str(path).strip() == '' or P.get('restart_reset_hazard', False):
        return False
    z = np.load(Path(path).expanduser(), allow_pickle=True)
    names = ('nuc_cand_active', 'nuc_cand_age',
             'nuc_cand_best_barrier', 'nuc_cand_birth_step')
    present = [name in z.files for name in names]
    if any(present) and not all(present):
        raise ValueError('checkpoint contains a partial candidate state')
    if not all(present):
        if (P.get('use_nuc_candidate_incubation', True)
                and not P.get('allow_legacy_candidate_restart_without_state', False)):
            raise ValueError(
                'legacy checkpoint omits candidate state; exact candidate restart is impossible')
        return False
    loaded = [np.asarray(z[name]) for name in names]
    if any(arr.shape != (Nx, Ny) for arr in loaded):
        raise ValueError('checkpoint candidate arrays do not match the current grid')
    nuc_cand_active = loaded[0].astype(bool, copy=True)
    nuc_cand_age = loaded[1].astype(np.int16, copy=True)
    nuc_cand_best_barrier = loaded[2].astype(float, copy=True)
    nuc_cand_birth_step = loaded[3].astype(np.int32, copy=True)
    if np.any(nuc_cand_age < 0):
        raise ValueError('checkpoint has negative candidate age')
    if np.any(nuc_cand_active & ~np.isfinite(nuc_cand_best_barrier)):
        raise ValueError('active candidate has no finite barrier history')
    return True
...
(_restart_loaded, eta, psi_gv, Ng, lab, psi_plastic, rp, rm, rho, T,
 ...
 grain_birth_barrier_eV) = _load_restart_state(
    ...
    grain_birth_theta_deg, grain_birth_theta_max_deg, grain_birth_R_um,
    grain_birth_barrier_eV)
_candidate_restart_loaded = _restore_candidate_restart_state(P.get('restart_file', None))
_restart_step_offset = 0
_restart_sim_time = 0.0
_restart_scalars = {}
if _restart_loaded and not P.get('restart_reset_clock', True):
    with np.load(Path(P.get('restart_file')).expanduser(), allow_pickle=True) as _restart_npz:
        _restart_step_offset = int(_restart_npz['step']) + 1 if 'step' in _restart_npz.files else 0
        _restart_sim_time = float(_restart_npz['sim_time']) if 'sim_time' in _restart_npz.files else 0.0
        for _runtime_name in ('rho_state_ref_runtime', 'rho_state_total_ref_runtime',
...
if P.get('use_rho_state_partition', False):
    ...
    P['_rho_state_ref_runtime'] = _restart_scalars.get('rho_state_ref_runtime', rho_ref_state)
    P['_rho_state_total_ref_runtime'] = _restart_scalars.get(
        'rho_state_total_ref_runtime',
        float(np.clip(np.nanmean(rho_total_init), P['rho_min'], P.get('rho_max', np.nanmean(rho_total_init)))))
    P['_rho_state_struct_ref_runtime'] = _restart_scalars.get(
        'rho_state_struct_ref_runtime',
        float(np.clip(np.nanmean(np.maximum(rho_struct_init, P['rho_min'])), P['rho_min'],
                      P.get('rho_max', np.nanmean(np.maximum(rho_struct_init, P['rho_min']))))))
...
def _gb_hp_rate_given_xi(seq, xi, T_field, gb_mask, extra_barrier_eV=None):
    ...
    channel = ('gb_transmission_activation_entropy_kB' if extra_barrier_eV is not None
               else 'gb_source_activation_entropy_kB')
    G = float(P.get('gb_hp_A_mult', 7.0)) * ATpot.barrier_H(seq_eff, Tloc)
    if extra_barrier_eV is not None:
        G = G + np.asarray(extra_barrier_eV, dtype=float)*eV_J
    free_barrier = G - kB_J*Tloc*float(P.get(channel, 0.0))
    rate = ATpot.eta0 * _activation_exponential_from_free_barrier(free_barrier, Tloc)
...
def _gb_slip_transmission_fields(lab, psi_field, gb_mask, T_field=None):
    ...
    trans_log = (float(P.get('gb_transmission_activation_entropy_kB', 0.0))
                 - best_bar/np.maximum(kBT_eV[:, :, None], 1e-12))
    fac = np.exp(np.clip(trans_log, -700.0, 0.0))
...
def _nuc_barrier_fields(rho, kappa_tot, rho_GB, gb_mask, psi_lat, T_field, activity_factor=None):
    ...
    rate = (float(P.get('nuc_attempt_freq', 1.0e6))*site*patch_weight*activity_factor*gate_AT*
            _activation_exponential_from_free_barrier(nuc_free_barrier,
                                                      np.maximum(T_field, 1.0)))
...
if _restart_loaded and not P.get('restart_reset_clock', True):
    with np.load(Path(P.get('restart_file')).expanduser(), allow_pickle=True) as _restart_npz:
        if 'sigma_bar' not in _restart_npz.files:
            raise ValueError('exact restart requires sigma_bar')
        sigma_bar = float(_restart_npz['sigma_bar'])
...
def _atomic_savez_compressed(path, **arrays):
    """Write a checkpoint beside its destination and publish with os.replace."""
    path = Path(path)
    tmp_name = None
    try:
...
if _restart_loaded and not P.get('restart_reset_clock', True):
    with np.load(Path(P.get('restart_file')).expanduser(), allow_pickle=True) as _restart_npz:
        if 'collective_activity_memory' not in _restart_npz.files:
            raise ValueError('exact restart requires collective_activity_memory')
        _memory_load = np.asarray(_restart_npz['collective_activity_memory'], dtype=float)
...
for n in range(_restart_step_offset, _restart_end_step):
    ...
    # --- SAVE FIELDS / FRAMES ---
    _save_due = (n % max(int(P.get('save_interval', 1)), 1) == 0 or n == _restart_end_step-1)
    _plot_due = (P.get('save_main_panels', True)
                 and (n % max(int(P.get('plot_interval', P.get('save_interval', 1))), 1) == 0 or n == _restart_end_step-1)
    ...
    if _save_due or _plot_due:
        ...
        if P.get('write_restart_npz', True) and (n % max(int(P.get('restart_interval', P['save_interval'])), 1) == 0 or n == _restart_end_step-1):
    ...
    if (P.get('write_restart_npz', True) and _wallclock_checkpoint_interval > 0.0
        ...
        if chk is not None:
            _last_checkpoint_wallclock = _wtime.time()
            if P.get('diag_print_extended', True):
                print(f"        wall-clock restart checkpoint: {chk}")
