...
def _complete_dissipation_pair(channel_ab, channel_ba):
    """Return nonnegative activities generated by each complete affinity.

    The two arguments may be distinct outgoing transactions rather than a
    microscopic reverse pair.  Such channels cannot consistently be assigned
    a deterministic velocity by subtracting their raw transition-state
    traffic: that can point toward an uphill complete endpoint.  Instead each
    geometric direction receives activity only from its own negative complete
    free-energy change.  ``1-exp(Delta F/kT)`` is the bounded thermodynamic
    force, while the retained EXP-floor rate supplies its nonnegative kinetic
    coefficient.  Every nonzero channel therefore has nonpositive ``Delta F``
    and the later complete-energy guard remains authoritative.
    """
    def activity(channel):
        affinity = channel.endpoint_free_energy_change_J/(
            KB_J_K*channel.temperature_K)
        force = -math.expm1(max(affinity, -700.0)) if affinity < 0.0 else 0.0
...
def propose_bidirectional_front_event(
    ...
    if deterministic_rate_law == "legacy_independent_metropolis":
        rate_ab, rate_ba = _directional_metropolis_pair(channel_ab, channel_ba)
    elif deterministic_rate_law == "complete_dissipation":
        rate_ab, rate_ba = _complete_dissipation_pair(channel_ab, channel_ba)
    else:
        raise ValueError("unknown deterministic front rate law")
    expected_log_ratio = (
