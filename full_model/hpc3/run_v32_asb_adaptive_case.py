#!/usr/bin/env python3
"""Run one fault-isolated V32 matched-control ASB screening case.

The initial tier changes exactly one thermomechanical coordinate from the
V31 anchor.  It deliberately leaves heterogeneity, heat conversion, process
zone, and every strict classification threshold fixed.
"""
...
# Four one-factor-at-a-time coordinates around the completed 1100 K, 3e4/s
# anchor. Each coordinate has its own isothermal matched control.
COORDINATES = (
    ("T0900_R3e4", 900.0, 3.0e4),
    ("T1300_R3e4", 1300.0, 3.0e4),
    ("T1100_R1e4", 1100.0, 1.0e4),
    ("T1100_R1e5", 1100.0, 1.0e5),
)


def case_definition(case_id: int) -> dict[str, object]:
    coordinate = case_id//2
    if coordinate < 0 or coordinate >= len(COORDINATES):
        raise ValueError(f"case_id must be in [0,{2*len(COORDINATES)-1}]")
    label, temperature, rate = COORDINATES[coordinate]
    thermal = "adiabatic" if case_id % 2 == 0 else "isothermal"
    return {
        "case_id": case_id,
        "case_name": f"{label}_seed43_{thermal}",
...
def main() -> None:
    ...
    parser.add_argument("--case-id", type=int, required=True,
                        choices=range(2*len(COORDINATES)))
    ...
    if actual_sha != args.expected_source_sha:
        raise RuntimeError(
            f"source SHA mismatch: expected {args.expected_source_sha}, found {actual_sha}")
    if subprocess.check_output(
            ["git", "status", "--porcelain"], cwd=source, text=True).strip():
        raise RuntimeError("adaptive ASB source worktree must be clean")
    ...
    if remaining == 0:
        print(f"{case['case_name']}: already complete at {completed} steps")
        return

    isothermal = case["thermal_control"] == "isothermal"
    parameters = {
    ...
    record = case | {
        "source_root": str(source), "source_commit": actual_sha,
        "restart_file": str(restart) if restart else None,
    ...
    (output/"v32_adaptive_run_record.json").write_text(
