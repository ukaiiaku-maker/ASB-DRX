...
The common-equation mechanism ladder changes only declared loading and mechanism controls. Disabling phase evolution holds both order fields exactly fixed and produces zero phase heat. Disabling temperature evolution is the infinite-conductance isothermal limit: generated mechanical and phase heat is transferred to an explicit bath term, so `Delta E_thermal + Q_bath = Q_mechanical + Q_phase`. Thermal cases are compared with automatically generated isothermal twins at the same applied rate and phase setting.
