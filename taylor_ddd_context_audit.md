...
## HPC3 structural diagnostic

HPC3 run `20260828T130729Z-1a147e0-710710` (job `55637740`, commit `1a147e0`) completed with both diagnostic tests passing and verified retrieval. Its six input hashes match the audited source files. The result is structural falsification context only and supplies no production parameter.

The two reduced EXP-floor histories show a pronounced density dependence. At `1e15 m^-2`, 15,680 events have inter-event-step CV `0.996`, multi-hit event fraction `0.00918`, and a same-step cluster proxy `0.00459`. At `1.5e16 m^-2`, 167,391 events have CV `1.080`, multi-hit event fraction `0.0981`, and same-step proxy `0.04999`. The arbitrary 10-step cluster proxy increases from `0.0908` to `0.6500`. These window-dependent values establish increased clustering in these simulations; they are not branching ratios and cannot parameterize the continuum model.
