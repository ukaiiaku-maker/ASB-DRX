...
- Submitted run `20260914T045532Z-567ba0a-c6d113` as Slurm job `56003778`; it entered `RUNNING` on `hpc3-14-16`. Scientific source is `873c3b03d7019807ce71b70755b4475a8ab58a01`, clean archive commit is `567ba0ada24c49e9da54c7a13a3fd05f1c76112a`, and source-archive SHA-256 is `91089b588eaa1ebb87577399594925a479ce92f9cc5e0376604330dee03540a4`.
- Slurm job `56003778` completed with exit `0:0` in `09:34:00`; all 17 cases were independently finalized. The 10 GB result was fetched with runner verification. The accepted result archive SHA-256 is `7b4f48ba4f8d93e1604db35525181b370f71b7b28aea630f08bfc2941eb8d047`.
- Decision-grade checkpoint auditing rejects the provisional wrapper classifications as production SIBM evidence. Every geometrically seeded trajectory has zero parent cells above the declared `eta >= 0.8` pure-core threshold in its step-0 campaign checkpoint. Both unseeded controls begin with resolved pair cores but lose the parent core by step 250. A surviving diffuse zero contour is therefore not an independently resolved physical grain pair.
- All 17 audited endpoints pass phase-simplex, population nonnegativity, line-content, signed-Burgers, energy/heat, and no-label/no-orientation-allocation checks. These conservation results do not repair the pair-identity hard-invariant failure.
- The baseline tip change minus its matched control is `-6.2679964e-7 m` at their common valid output horizon. The 2.0 and 3.5 micrometre window trajectories agree to much less than one 128-grid cell, but only as pair-invalid secondary morphology. The 128/192/256 tip-change spread is `80.225%`, failing the provisional 5% comparison, and the mobility trend is not ordered.
- The resolvable-criticality fixture fails: both seeded initial states lack a resolved parent core, and both endpoints retreat under negative local pressure. In particular, the nominal supercritical E2 case does not grow. Temperature and strain-rate effects are not promoted as physical trends from this invalid pair state.
- Directive-v12 decision: `FULL_MODEL_SIBM_GROWTH_MECHANISM_NOT_SUPPORTED`. The matrix is retained as implementation-falsification and negative-mechanism evidence. Before another range campaign, the source/seed must preserve independently resolved parent and child cores, and pair-core loss must become a clean per-case stop.
- The decision postprocessor has five direct unit tests; the complete configured repository suite passes `325` tests. All seven postprocessing artifacts have recorded SHA-256 checksums.

## Directive v13 classification amendment (2026-09-14)

- Directive v13 supersedes the broad v12 mechanism-level rejection. No raw trajectory or historical record is rewritten.
- Amended decisions: `V12_SOURCE_PAIR_AND_POST_SEED_GEOMETRY_INVALID_FOR_SIBM_QUALIFICATION` and `FULL_MODEL_SIBM_GROWTH_MECHANISM_UNRESOLVED`.
- The v12 matrix falsifies the selected step-6900 pair and disk-overwrite initialization, not the common SIBM mechanism. Common-operator fixtures, conservation ledgers, exact checkpointing, fault-isolated campaign execution, and no-allocation behavior remain supported.
- Post-seed pair identity is now a pre-step hard invariant with physical depth, connected-core inradius, HAGB length, triple-junction, other-boundary, and active-window clearances reported in metres. The retired step-6900 source writes `SIBM_INVALID_POST_INITIALIZATION_PAIR` and exits before step 1.
- Disk overwrite is replaced by a signed-distance displacement of the existing pair contour with a pinned circular-cap or cosine profile. Zero amplitude is bitwise identical, non-pair fields are preserved, and the phase simplex closes.
- The actual two-dimensional pinned-cap energy now reports arc, replaced chord, swept area, curvature, stored work, compatibility, drag, numerical penalty, total energy, and its analytical amplitude derivative. The derivative agrees with finite differences; at amplitude `0.6 um`, half-chord `1.5 um`, and `gamma=0.5 J/m2`, the derived critical stored pressure is `229885.057 Pa`.
- Controlled 128 and 192 square bicrystal fixtures retain broad post-seed pure cores and pass all physical geometry criteria. A two-step canonical production-driver run completes with exact line, signed-Burgers, and energy/heat closure and no allocation.
...
## Directive v14 conservation repair and local criticality (2026-09-14)

- Scientific repair commit `b0bddea788f0ac54fc4bcf7cc9f3b565577263ac` replaces the independent latent recovered-child target with support-owned parent/child/wake state and a constrained per-family, per-sign transfer. The v13 infeasible operator remains quarantined solely for reproducing the frozen failure audit.
- Positive/negative transmission, neutral annihilation, excess-boundary storage, neutral sink, and the explicitly labeled signed-sink ablation close line and signed-Burgers balances cellwise. Intrinsic HAGB energy and numerical compatibility penalties are excluded from excess content and heat.
- A finite boundary capacity returns an accepted fraction and capacity-limited volume without clipping. A zero-capacity manufactured case stalls without mutating state. Diffuse profile relaxation is separated from signed normal-contour sweep; retreat and revisit do not process virgin content twice.
- The exact common 128 and 192 states have `chi=processed_max`, zero cumulative sweep ledgers, and verified checkpoint hashes recorded in `v14_common_checkpoint_128.json` and `v14_common_checkpoint_192.json`.
- From each common state, neutral, subcritical, supercritical, mobility-off, and reversed branches pass. Neutral drift is `-8.77e-5` cell at 128 and `-1.039e-2` cell at 192, within the declared `0.02`-cell contour tolerance. Subcritical and reversed branches retreat; supercritical branches advance; mobility-off is exactly stationary with an exactly zero sweep ledger after the zero-mobility guard.
- All moving cases close line, family-signed Burgers, released line energy, and heat ledgers and preserve the 12 labels and their orientations. Continuous 80-step versus 40+40-step continuations agree to maximum scaled differences `1.93e-13` and `2.05e-13`, within the declared `1e-12` numerical exact-continuation tolerance.
- A frozen-state finite difference of the same gradient, multi-order barrier, common stored-energy, compatibility, applied-pressure, and rho/eta phase functional matches its analytical directional derivative to `3.20e-7` relative or better. The inferred diffuse critical pressures are `0.8796 MPa` at 128 and `4.6205 MPa` at 192; both lie inside the observed `[-4.5, +5.5] MPa` motion-sign bracket. This large finite-interface/grid correction is reported, not tuned away.
- Machine result `full_model/verification/v14_pinned_criticality.json` reports `fixture_passed=true` and `scientific_gate_passed=true`. The matched v32/full-v34 ASB regression remains passed, and the complete configured suite passes `338` tests.
- The local authorization gate for one compact, non-array HPC3 bundle is passed. No array, rate/temperature sweep, or HPC result is claimed at this checkpoint.

## Directive v14 compact HPC3 qualification (2026-09-14)

- Prepared run `20260914T184004Z-777fa08-a20d64` / job `56013885` is retained as failed packaging evidence. It exited in eight seconds because `sibm_geometry.py` was omitted from the source archive. All 20 moving-front invariant tests passed on the compute node, but each full-driver case stopped at import and no scientific trajectory completed. The partial diagnostic archive was fetched without promoting it as verified scientific output.
- The missing runtime dependency alone was added and pushed at `c292ef7d58070b4a9f5215b49b5d057405fa383c`; no equation, initial state, pressure, duration, grid, or acceptance threshold changed.
- Corrected non-array run `20260914T184138Z-c292ef7-eb2b2a` / job `56013924` completed all five fault-isolated cases with exit zero on `hpc3-17-05` in `00:03:28`. The runner fetched and checksum-verified the result archive (`2f2721a657cba737ffa30f195673bf1edf88fc6873c8d41ed08dd3437885e31c`). Unrelated job `55950433` was preserved.
